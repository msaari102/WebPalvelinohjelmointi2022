class BeerClubsController < ApplicationController
  ...
  def show
    ...
    @membership_confirmed = @beer_club.memberships.where(confirmed: true)
    @membership_applied = @beer_club.memberships.where(confirmed: [false, nil])
    @is_member = !Membership.where(user_id: current_user).where(confirmed: true).empty?
  end
